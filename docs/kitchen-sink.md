...
| 7 | Rich Text | `Paragraph`, `Markdown` (its §10 **embeds** a `Mermaid` flowchart + a `Structurizr` C4 model inline via `Markdown::diagrams`), `Mermaid`, `Structurizr` (C4), `JsonCanvas` (explicit placement) |
