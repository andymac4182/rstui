...
| | `DataTable` | `hit(area,pos) -> Option<DataTableHit>` (`Header(col)`) + `cell_rect`; `DataTableState::reorder_column(from,to)` keeps sort/group keys on the moved column when the caller permutes its `Vec<DataColumn>` |
