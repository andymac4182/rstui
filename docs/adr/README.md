...
| [0023](0023-treesitter-tier1-excluded-leaf-crate.md) | tree-sitter Tier-1 as a workspace-`exclude`d opt-in leaf crate (`rstui-treesitter`) — refines ADR 0022's mechanism so the `--all-features --workspace` gates never compile it; decision unchanged | Accepted |
