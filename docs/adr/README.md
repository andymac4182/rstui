...
| [0023](0023-treesitter-tier1-excluded-leaf-crate.md) | tree-sitter Tier-1 as a workspace-`exclude`d opt-in leaf crate (`rstui-treesitter`) — refines ADR 0022's mechanism so the `--all-features --workspace` gates never compile it; decision unchanged | Superseded by [0024](0024-code-widget-crate-and-treesitter-exemption.md) |
| [0024](0024-code-widget-crate-and-treesitter-exemption.md) | `rstui-code` widget crate (move `Editor`/`Diff`/code modules + fold the tree-sitter engine in) with a first-class, gate-protected tree-sitter exemption — supersedes ADR 0023, amends ADR 0022 (Tier-1 mechanism) & ADR 0002 (scoped exemption); only `rstui-code` consumers pull tree-sitter | Accepted |
| [0025](0025-caller-owned-line-cache.md) | Caller-owned line cache (`LineCache` seam: `DiagramCache`/`MarkdownCache`) — the one sanctioned memo for an immediate-mode widget's heavy per-frame parse+layout; opt-in, byte-identical, model-owned (not a retained tree); amends ADR 0012 by naming its caching seam (perf-review-3 R3-1/R3-2) | Accepted |
| [0026](0026-calendar-app-widget-suite.md) | Calendar-app widget suite — one shared caller-owned `CalendarEvent` model (+ `pack_day` overlap packer) and nine pure-projection views/controls (Month/Week/Day/Agenda/Year, TimePicker, EventCard, EventEditor, DateNavigator); no date math (ADR 0002 §4), the app owns move/schedule via hit accessors (ADR 0012); purely additive | Accepted |
