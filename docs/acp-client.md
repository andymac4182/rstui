...
| 5 | Agent-driven UI — the `initialize` `_meta` advertises the renderable catalog (A2UI inline catalog + json-render catalog/prompt); an agent-sent A2UI / json-render document is detected and rendered in the transcript via `rstui-jsonui`, and is **interactive** — a click round-trips a server `event` to the agent and a two-way control (checkbox/tab/`setState`) mutates a caller-owned stateful `RichDoc` that **persists** across the every-frame re-projection; **`/render [request]`** primes *any* agent with the json-render format + catalog as a prompt (works without `_meta` support) ([ADR 0017](adr/0017-ai-app-widgets-and-declarative-agent-ui.md), [Agent UI](agent-ui.md)) |
