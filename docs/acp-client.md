...
| 5 | Agent-driven UI — the `initialize` `_meta` advertises the renderable catalog (A2UI inline catalog + json-render catalog/prompt); an agent-sent A2UI / json-render document is detected and rendered in the transcript via `rstui-jsonui`, and is **interactive** — it opens in a **live pane on the right** of the chat with keyboard focus (`Tab` in/out, focus-ring nav, type into bound fields, `Enter` submit) and mouse; a click/submit round-trips the **exact spec envelope** (A2UI `{action:{…,context}}` / json-render `{action,params}`) with typed values resolved from the now-updated model, a two-way control (checkbox/tab/`setState`) mutates a caller-owned stateful `RichDoc` that **persists**, and an agent follow-up for the same surface folds back into the open pane (loop closed); **charts** (Bar/Line/Pie/Sparkline/Scatter/Histogram/StackedBar/Heatmap) and **theme-token colours** work in both formats; **`/render [request]`** primes *any* agent with the json-render format + catalog as a prompt (works without `_meta` support) ([ADR 0017](adr/0017-ai-app-widgets-and-declarative-agent-ui.md), [Agent UI](agent-ui.md)) |
...
| 10 | Composer **readline / emacs editing** — the message composer answers the full GNU-readline key vocabulary every shell user already has in their fingers. Caret motion: `Ctrl+A`/`Ctrl+E` line start/end, `Ctrl+B`/`Ctrl+F` by character, `Alt+B`/`Alt+F` (and `Ctrl`/`Alt`+`←`/`→`) by word, `Alt+<`/`Alt+>` to the composer ends. A **kill ring**: `Ctrl+W` (whitespace-word) / `Alt+⌫` (word) / `Ctrl+U` kill backward, `Ctrl+K` / `Alt+D` kill forward — consecutive kills coalesce, `Ctrl+Y` yanks, `Alt+Y` yank-pops through the ring. `Ctrl+T` / `Alt+T` transpose characters / words; `Alt+U`/`Alt+L`/`Alt+C` upper/lower/capitalize the word; `Alt+\` deletes surrounding whitespace. **Per-line undo** on `Ctrl+_` (a typing run is one step; the kill ring is global, undo is reset per recalled line) and `Alt+R` revert-line. Plus `Ctrl+P`/`Ctrl+N` history, `Ctrl+D`/`Ctrl+H` delete, `Alt+.` yank-last-arg, `Ctrl+L` redraw, `Ctrl+G` dismiss-popup, `Ctrl+J` accept-line. **Incremental history search** on `Ctrl+R` (reverse) / `Ctrl+S` (forward): the typed query substring-matches a past prompt shown live in the composer, `Ctrl+R`/`Ctrl+S` step through matches, the block title shows the `` (reverse-i-search)`query': `` prompt, `Ctrl+G` aborts (restoring the draft) and `Enter` accepts *and* submits. The editing logic lives in the unit-tested `readline` module ([`ReadlineState`] — kill ring + undo + last-command bookkeeping); the keys are wired raw in `chat_key` (ADR 0015). The keymap editor moved off `Ctrl+K` (which the composer now claims for `kill-line`) to **`Ctrl+X`** |
| 11 | **Auto-growing composer** — the message input box is no longer a fixed 5-row box. It grows a row at a time as the draft gains lines or a long line wraps, and shrinks back as text is removed, down to a one-row minimum; it is capped so the transcript always keeps room, and past the cap the composer scrolls to keep the caret visible. The sizing — [`composer_box_height`](../crates/rstui-acp-client/src/ui.rs) plus the caret/scroll measures — is a pure function of the draft, shared by the renderer and the click hit-test so the transcript/composer split can never desync (ADR 0012). |
