...
| 7 | [Codex-CLI parity](acp-client-codex-parity.md) Wave 2 (ACP-wired) — **W2-1** `/status` overlay: session config (agent, cwd, connection, model, theme, keymap, history, bell) + **token usage** folded from the ACP `usage_update` notification (tokens in context + % of the window); **W2-2** `/model` picker — the agent's `NewSessionResponse.models` catalogue, Enter issues `session/set_model`, the `ModelSelected` ack updates the active model |
