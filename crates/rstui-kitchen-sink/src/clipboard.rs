...
//!
//! There is no clipboard crate (the workspace forbids casual deps, ADR
//! 0001/0003) and crossterm has no clipboard API, so "copy" is done the way
//! every terminal app does it: emit `ESC ] 52 ; c ; <base64> BEL`, which the
//! terminal turns into a real host-clipboard write (iTerm2, kitty, wezterm,
//! Ghostty, Alacritty, tmux with `set-clipboard on`, …). Reading the
//! clipboard back over OSC 52 is unreliable and often disabled, so *paste*
//! comes the other direction — the terminal's own paste gesture arrives as a
//! bracketed [`Event::Paste`](rstui_core::Event::Paste) — and is handled by
//! the app, not here.
//!
...
/// still keeps its own in-app clipboard and shows feedback regardless).
pub(crate) fn copy(text: &str) -> bool {
    // Don't touch a terminal that isn't there (tests, pipes): the in-app
    // clipboard + toast still happen; only the OS-clipboard hop is skipped.
