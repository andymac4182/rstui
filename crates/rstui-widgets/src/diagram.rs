//! The shared deterministic character grid every diagram widget lays itself
//! out on before a single centred, clipped blit to the [`Buffer`].
//!
//! This is the common drawing component reused across the diagram widgets so
//! they do not each reinvent box/line/text rasterisation: every non-flowchart
//! [`Mermaid`](crate::Mermaid) renderer (the flowchart renderer keeps its own
//! `Canvas`/`CellRole` per-node skin cascade the others do not need) and the
//! [`Structurizr`](crate::Structurizr) C4 renderer build onto this
//! [`Surface`] — a flat `(char, Style)` grid with integer-only primitives, so
//! each renderer is plain arithmetic and the result is one snapshot-testable
//! image. Out-of-bounds writes are silently dropped, so a layout that
//! overflows its computed size degrades to a clip rather than a panic — the
//! same leniency the parsers use. Crate-internal (`pub(crate)`), not a
//! public API surface.
//!
...
pub(crate) enum BoxStyle {
    ...
    Heavy,
    /// Square corners with dotted edges — a C4 boundary / soft container.
    Dashed,
}
