...
//! with optional **per-column in-cell editing**. The "spreadsheet pane" to
...
//!   `offset`.
//! * *Editing* is a borrowed caller-owned [`TextEdit`],
//!   exactly the [`Input`] model: when [`DataTableState`] says a
//!   cell is being edited, the widget renders that one cell **by reusing
//!   [`Input`]** (one caret/scroll implementation, not a second). The reducer
//!   owns the keystrokes and the write-back.
            ...
            match vrow {
                ...
                VisualRow::Data { source } => {
                    ...
                    for (ci, rect) in columns.iter().enumerate() {
                        ...
                        let editable = self.columns.get(ci).is_some_and(DataColumn::is_editable);
                        let editing_here = self.state.is_editing(*source, ci);
                        if editing_here && editable {
                            if let Some(te) = self.edit {
                                // Reuse Input for the one edited cell — a
                                // single caret/scroll implementation.
                                ...
                                field.render(Rect::new(rect.x, y, cell_w, 1), buf);
                                continue;
                            }
                        }
