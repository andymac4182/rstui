//! `sequence` Mermaid diagram renderer (scaffold stub).
//!
...
/// Renders a `sequence` Mermaid diagram from `src` into `area`.
pub(crate) fn render(
    _src: &str,
    area: Rect,
    buf: &mut Buffer,
    ...
    super::diagram_placeholder("sequence", "renderer pending", area, buf, base, theme);
}
