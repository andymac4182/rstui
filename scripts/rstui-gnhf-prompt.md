...
5. If the repository is still empty, first create a minimal Rust 2024 workspace foundation with README, CI, crate layout, and a tiny runnable example. Keep the foundation small and useful.
6. Plan the workspace around real boundaries: core runtime, renderer/layout/style primitives, components, examples, and plugin host/runtime. Introduce crates when there is enough real API surface to justify the boundary; avoid empty placeholder crates.
7. Use current stable crates where they clearly help, but do not add dependencies speculatively. Prefer simple, testable Rust APIs and keep public names coherent.
8. Add focused tests or examples for each new public capability. For behavior that is hard to test directly, add a small deterministic model or snapshot-style check.
9. Run the appropriate validation before reporting success. For Rust code, prefer cargo fmt --all --check, cargo clippy --all-targets --all-features -- -D warnings, and cargo test --all-features once those commands exist.
10. Update notes.md with the slice completed, upstream facts learned, and the next likely surface to build.
