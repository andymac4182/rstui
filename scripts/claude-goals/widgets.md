...
one agent can inspect OpenTUI/OpenCode/gpui-component patterns, one can
implement a focused widget slice, and another can verify tests/examples/docs.
Keep ownership clear and integrate all agent work in this stream worktree
before committing. Do not spawn agents to edit other streams' owned areas.
...
```sh
git -C "$main_repo" merge --no-ff "$branch" -m "Merge widgets goal slice"
(
  cd "$main_repo"
  cargo fmt --all --check
  cargo clippy --all-targets --all-features -- -D warnings
